...
const checkRestaurantIsPinned = async (req, res, next) => {
  try {
    const restaurant = await Restaurant.findByPk(req.params.restaurantId)
    if (restaurant.pinned) {
      return next()
    }
    return res.status(409).send('This restaurant is not pinned.')
  } catch (err) {
    return res.status(500).send(err.message)
  }
}

export { checkRestaurantOwnership, restaurantHasNoOrders, checkRestaurantIsPinned }
